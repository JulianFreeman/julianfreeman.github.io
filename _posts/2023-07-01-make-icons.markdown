...
Inkscape 并不会把自己的 `bin` 目录自动添加到环境变量，所以需要手动添加一下。该目录通常是 `C:\Program Files\Inkscape\bin`。
...
其中 `icon.svg` 就是我们提前准备的 SVG 图标文件了。此命令在该文件的同级目录下生成一个 `1024.png` 文件。

## PNG to ICNS

打开 Image2Icon 软件，将刚才创建的 `1024.png` 文件拖放进软件界面，点击”导出“，选择 ICNS 格式，”保存图标“就可以了。

---

封面图：Photo by [Harpal Singh][cover-img-author] on [Unsplash][cover-img-src]
