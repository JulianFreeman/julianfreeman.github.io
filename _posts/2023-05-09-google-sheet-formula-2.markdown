...
`FILTER` 接受两个基本参数，第一个是过滤范围，第二个是过滤条件，第三个参数虽然名为“用于指明范围中的哪些行或列应该进行过滤”（[官方文档][filter-doc]），其实不就是另一个过滤条件么。而且第三个参数是可重复的，所以该函数其实可以拥有多于两个的过滤条件。
...
例子这里就不举了，基础教程可以查看 [官方文档][map-doc]。这里主要说一下映射范围的一些细节。

一般来说，传递给 `MAP` 的要进行 lambda 计算的范围是一个单列或者单行。如果是一个单列，那么每一列的值进行 lambda 计算之后的结果只能是单行，不能是多行。同样，如果范围是一个单行，那么每一行的值进行 lambda 计算后的结果只能是单列，不能是多列。
